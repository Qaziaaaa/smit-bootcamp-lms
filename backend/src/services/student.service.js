...
const getStudents = async (filters = {}, pagination = {}) => {
  const { search, batch, teamId } = filters;
...
const updateStudent = async (id, data) => {
  ...
  if (email) {
    ...
    const existingUser = await User.findOne({
      email: email.toLowerCase(),
      _id: { $ne: (await Student.findById(id)).userId },
    });
    if (existingUser) {
      throw new ApiError(409, 'Email already exists.', ['A user with this email already exists.']);
    }
  ...
  const student = await Student.findByIdAndUpdate(id, { ...rest, ...(email && { email: email.toLowerCase() }) }, { new: true, runValidators: true }).populate('teamId', 'name');
