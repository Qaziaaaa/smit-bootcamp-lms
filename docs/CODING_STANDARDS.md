...
- Validate all input (see API standards). Use a validation library (e.g. `express-validator` or `joi`).
- Hash passwords with `bcrypt` (min 10 salt rounds). Never store plaintext.
